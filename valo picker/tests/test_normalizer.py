...
from valo_picker.models import NormalizationIssueKind, SelectionState
...
def has_issue(issues, kind: NormalizationIssueKind, **params) -> bool:
    return any(issue.kind == kind and all(issue.params.get(key) == value for key, value in params.items()) for issue in issues)
...
class PreGameNormalizerTests(unittest.TestCase):
    def test_normalizes_valid_ascent_payload(self):
        ...
        self.assertEqual(
            ...
            [
                ("Player 1", "Jett", SelectionState.LOCKED, False),
                ("Player 2", "Reyna", SelectionState.SELECTED, False),
                ("Player 3", "Killjoy", SelectionState.LOCKED, False),
                ("Player 4", "Sova", SelectionState.LOCKED, False),
                ("You", None, SelectionState.NONE, True),
            ],
        )
    ...
    def test_unknown_map_id_returns_warning(self):
        ...
        self.assertTrue(has_issue(result.warnings, NormalizationIssueKind.UNKNOWN_MAP, map_id="/Game/Maps/Unknown/Unknown"))
        self.assertEqual(result.errors, ())
    ...
    def test_unknown_character_id_returns_slot_without_agent_and_warning(self):
        ...
        self.assertTrue(has_issue(result.warnings, NormalizationIssueKind.UNKNOWN_CHARACTER, index=1, character_id="unknown-character-id"))
        self.assertEqual(result.errors, ())
    ...
    def test_unknown_selection_state_returns_none_and_warning(self):
        ...
        self.assertTrue(has_issue(result.warnings, NormalizationIssueKind.UNKNOWN_STATE, index=2, state="hovered"))

    def test_missing_ally_team_players_returns_error(self):
        result = normalize_pregame_match(pregame_payload(AllyTeam={}))

        self.assertEqual(result.team, ())
        self.assertTrue(has_issue(result.errors, NormalizationIssueKind.MISSING_ALLY_PLAYERS))
    ...
    def test_missing_ally_team_returns_error(self):
        ...
        self.assertEqual(result.team, ())
        self.assertTrue(has_issue(result.errors, NormalizationIssueKind.MISSING_ALLY_TEAM))
